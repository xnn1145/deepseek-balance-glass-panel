...
- 📊 **今日会话花销（余额记账法）**：与余额同一把 `DEEPSEEK_API_KEY`——每次观测余额后按**差值自动记账**（持久化到 `~/.dsh/.dsh-glass-usage.json`，跨天自动归零归档）
...
- **外部模型花销算得不对**：点面板右上角**齿轮**，按你实际被收取的价格填写「输入（未命中）/命中（缓存）/输出」每 1M Token 价格；高峰期按「高峰倍率」计（外部模型通常是统一价，填 1 即可）。保存后立即生效并自动刷新。
