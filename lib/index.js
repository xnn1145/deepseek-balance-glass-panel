...
// 官方峰谷计费规则调整(2026-08-23 00:00 北京时间起):周末(周六/周日)全天不再区分峰谷,
// 统一按低谷(空闲)时段价格计费。此前的周六/周日仍按工作日峰谷时段计费。
const WEEKEND_FLAT_START_MS = Date.UTC(2026, 7, 22, 16, 0, 0) // = 2026-08-23T00:00:00+08:00
...
function isBeijingWeekend(ms) {
  const dow = new Date(Number(ms) + 8 * 3600000).getUTCDay()
  return dow === 0 || dow === 6
}

function isPeakTime(ms) {
  const t = Number(ms)
  if (t >= WEEKEND_FLAT_START_MS && isBeijingWeekend(t)) return false
  const hour = new Date(t + 8 * 3600000).getUTCHours()
