...
function apply(ctx) {
  ...
  function snapshotStats(sid, e, price) {
    ...
    const costOf = (b, peak) => (b.miss * price0.miss[peak ? 1 : 0] + b.hit * price0.hit[peak ? 1 : 0] + b.out * price0.out[peak ? 1 : 0]) / 1000000
  ...
  async function getStats(force, price) {
    ...
    const stats = snapshotStats(currentSid, e, price)
    ...
    if (hasLive) {
      savedSnap = {
        ...
        todayBuckets: stats.todayBuckets,
        todos: stats.todos,
        task: stats.task,
      }
  ...
  disposers.push(ctx.webServer.register({
    ...
    handler: async (req, res) => {
      try {
        ...
        const payload = await getStats(force, price)
