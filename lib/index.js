...
const WIDGET_JS = `(function () {
  ...
  function load(force) {
    if (force) state.refreshing = true
    fetch(STATS_URL + (force ? '?refresh=1' : ''), { cache: 'no-store' })
      ...
      .finally(function () { if (force) { state.refreshing = false; render() } })
  }
...
function apply(ctx) {
  ...
  async function fetchBalance() {
    const now = Date.now()
    if (balanceCache.payload && now - balanceCache.at < BALANCE_TTL_MS) return balanceCache.payload
  ...
  function snapshotStats(sid, e) {
    ...
    const peak = e ? { cost: costOf(e.state.peak, true), tokens: tokensOf(e.state.peak) } : { cost: 0, tokens: 0 }
    const idle = e ? { cost: costOf(e.state.idle, false), tokens: tokensOf(e.state.idle) } : { cost: 0, tokens: 0 }
    const tPeak = e ? { cost: costOf(e.today.peak, true), tokens: tokensOf(e.today.peak) } : { cost: 0, tokens: 0 }
    const tIdle = e ? { cost: costOf(e.today.idle, false), tokens: tokensOf(e.today.idle) } : { cost: 0, tokens: 0 }
    if (tokens === null && e) {
      tokens = {
